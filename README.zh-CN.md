...
- 支持在日历中拖动 vault Markdown 任务卡片改期；开启写回后，也支持拖动 Apple Reminders 改到另一天。
- 支持只读公共 ICS 日历源。
...
日历视图会合并有日期的任务、公共 ICS 事件、Apple Calendar 事件，以及可用的有日期 Apple Reminders。你可以在月、周、日布局之间切换。把 vault Markdown 任务卡片拖到另一天，会更新该任务现有的 `📅 YYYY-MM-DD` 或 `due:: YYYY-MM-DD` 日期。开启 Apple Reminders 完成状态写回后，也可以拖动有日期的 Apple Reminder 卡片来修改提醒日期。
...
- Apple Reminders 完成状态和日期写回是可选能力，需要在设置中单独开启。
